...
with conn() as db:
    ...
    ''').fetchall()
topics = [dict(r) for r in rows if (r['priority_tier'] or 2) == 1 or r['category'] == 'presentation']
print(f'[dump] {len(topics)} high-priority topics', flush=True)
out = []
for i, r in enumerate(topics):
    try:
        ...
        chunks = []
        print(f'[dump] retrieval error on {r["topic"]}: {e}', flush=True)
    ...
    if (i + 1) % 25 == 0:
        print(f'[dump] {i+1}/{len(topics)}', flush=True)
json.dump(out, open('data/_kp_retrieval.json', 'w', encoding='utf-8'), ensure_ascii=False)
print(f'[dump] DONE -> data/_kp_retrieval.json ({len(out)} topics)', flush=True)
